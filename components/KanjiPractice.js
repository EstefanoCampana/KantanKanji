...
import { getKanji } from "@/app/services/kantankanji-services";
import { GrCaretNext, GrCaretPrevious } from "react-icons/gr";

export default function KanjiPracticeComp() {
    ...
    useEffect(() => {
        if(mode == null || kanjiArray.length === 0) return;
        let random = Math.floor(Math.random() * kanjiArray.length);
        let sliced = [...kanjiArray.slice(random, random + inputNumber)]
        if(sliced.length < inputNumber){
            sliced = [...sliced,...kanjiArray.slice(0,inputNumber - sliced.length)]
        }
        const shuffled = sliced.sort(() => Math.random() - 0.5);
        setQuestionsArray(shuffled);
        setCounter(0);
    }, [mode, kanjiArray, inputNumber])
   ...
   const answersArray = useMemo(() => {
        if(counter == null || !questionsArray[counter] || kanjiArray.length === 0) return;
        const correct = currentQuestion.reading;
        const incorrect = kanjiArray.filter(p => p.reading !== correct).sort(() => Math.random() - 0.5).slice(0,3).map(k => k.reading);
        return [correct, ...incorrect].sort(() => Math.random() - 0.5);
    }, [currentQuestion, kanjiArray])
    ...
    const handleKeyDown = (event) => {
        if (event.key === "Enter") {
            if (inputText === questionsArray[counter].reading) {
                resetTimer();
                setInputText("");
            ...
            {
                let wrongAnswer = {
                    readingG: questionsArray[counter].reading,
                    readingW: inputText,
                    character: questionsArray[counter].character
                }
    ...
    const handleInput = (reading) => {
        if (reading === questionsArray[counter].reading)
        {
            resetTimer();
            setCounter(prev => prev + 1);
            setResultCount(prev => prev + 1);
    ...
    return (
        <div className="flex flex-col bg-amber-50 md:px-10 p-4 rounded-2xl w-full">
            {mode == null && (
                <div className="flex flex-col bg-gray-300 rounded-2xl p-5 items-center">
                    ...
                    <div className="flex flex-col md:flex-row md:p-2 justify-center items-center bg-white rounded-lg ">
                        <label className="text-sm text-black pr-1">No. Kanji: </label>
            ...
            {mode === "easy" && (
                <div className="flex flex-col w-full">
                    <div className="flex flex-row text-black ">
                        <p className="text-md">{counter+1}. {translation?.charAt(0).toUpperCase() + translation?.slice(1)}</p>
                    ...
                    <div className="w-full flex p-4 my-4 self-center items-center justify-center bg-gray-300 rounded-2xl border-2 border-black">
                        {currentQuestion && <p className="text-center text-black font-bold">{currentQuestion.character}</p>}
                    </div>
                    <div className="grid grid-flow-dense grid-rows-2 grid-cols-2 gap-2">
                        {answersArray?.map((reading, id) => (
                            <button value={reading} onClick={() => handleInput(reading)} className="py-1 rounded-md bg-red-500 border-gray-200 border-2 hover:bg-red-500/50" key={id}>{reading}</button>
                        ))}
                    </div>
                </div>
            )}
            {mode === "hard" && (
                <div className="flex flex-col w-full">
                    <div className="flex flex-row text-black ">
                        <p className="text-md">{counter+1}.</p>
                        <p className="ml-auto font-bold">{(totalTime / 1000).toFixed(0)}</p>
                    </div>
                    ...
                    <div className="flex items-center justify-center">
                        <input maxLength={8} autoCapitalize="none" autoCorrect="off" placeholder="入力" onChange={handleTextInput} onKeyDown={handleKeyDown} value={inputText} className="border-2 border-black align-middle text-center text-black w-24 pb-[env(safe-area-inset-bottom)]"></input>
                    </div>
                </div>
            )}
            {mode === "results" && (
                        ...
                        <div className="flex flex-row m-4">
                            ...
                            <div className="flex flex-col md:flex-row w-full max-w-md border border-black rounded-2xl p-3 gap-3">
                                <div className="flex flex-col">
                                    ...
                                    <p className="text-sm text-center">{resultsArray[resultsArrayInd]?.character}</p>
                                </div>
                                <div className="flex flex-col">
                                    <p className="text-sm font-bold text-center">Your Answer</p>
                                    <p className="text-sm text-center">{resultsArray[resultsArrayInd]?.readingW === 0 ? "‎" : resultsArray[resultsArrayInd]?.readingW}</p>
                                </div>
                                <div className="flex flex-col">
                                    <p className="text-sm font-bold text-center">Actual Answer</p>
                                    <p className="text-sm text-center">{resultsArray[resultsArrayInd]?.readingG}</p>
                                </div>
                            </div>
